...
// Imagenes
	...
	(variables -> enemigoimg[6])  = al_load_bitmap("imagenes/pinchos.png");
	(variables -> enemigoimg[7])  = al_load_bitmap("imagenes/bala.png"); 
...
int inicializar_sv (variablesservidor *varsv){

    // Inicializo sockets
    (varsv -> sockfd) = socket(AF_INET, SOCK_STREAM, 0);

    if ((varsv -> sockfd) < 0) 
        error("ERROR opening socket");

    // Armando la estructura "sockaddr_in"
    (varsv -> serv_addr).sin_family = AF_INET;
    (varsv -> serv_addr).sin_addr.s_addr = INADDR_ANY;
    (varsv -> serv_addr).sin_port = htons(PUERTO);
    memset((void *) &((varsv -> serv_addr).sin_zero), '\0', 8); // Poner a cero el resto de la estructura

    if (bind((varsv -> sockfd), (struct sockaddr *) &(varsv -> serv_addr), sizeof(varsv -> serv_addr)) < 0)
        error("ERROR on binding");

    listen(varsv -> sockfd,5);
    (varsv -> clilen) = sizeof(varsv -> cli_addr);
